...
**Workflow 与消费者。** `ci-push.yml` 承载 `push` 到 `dev` 与 `workflow_call`,运行 engineering/current。PR 调用固定传入候选 SHA,被调用 jobs 检出该候选且缓存只读。`ci-pr.yml` 只承载 `pull_request`,目标分支保持 `dev` 与 `integration-**`;每轮使用 native `github.sha=M` 及其第一父 `B=M^1`,将不可变身份传给调用的 push 流程与 delta。所有检查 jobs 均检出同一个 M,B 只供 delta 使用;各 job 不得重新解析可变 ref;冲突 PR 不触发该事件。delta 依赖两个共享阶段的成功与产物交接,不再运行 engineering/current。
